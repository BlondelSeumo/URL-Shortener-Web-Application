...
class ValidateDomainOwnershipRule implements Rule
{
    ...
    public function passes($attribute, $value)
    {
        ...
        if (
            ...
            ->when(($this->user->can('globalDomains', ['App\Link', $this->user->plan->features->global_domains]) || (config('settings.short_domain') && config('settings.short_domain') == $value)), function($query) use ($value) {
